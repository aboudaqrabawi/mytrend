...
import {height} from '../Map/MapScreen';

export default class ReportAuserProblem extends Component {
  constructor(props) {
    super(props);
    this.state = {
      status: 200,
  ...
  async getReasons() {
    ...
    let response = await fetch(config.DOMAIN + 'problem/reasons', {
      method: 'get',
      headers: {
  ...
  async SendReport() {
    ...
    let response = await fetch(config.DOMAIN + 'problem/add', {
      method: 'post',
      headers: {
        Authorization: 'Bearer ' + userId,
        Accept: 'application/json',
      ...
      <ListItem selected={false} style={{marginTop: '5%'}}>
        <Left>
          <Radio
            ...
            onPress={() => this.select(item.reason_en, index)}
